...
int check_previously_known_nodes(const client_settings_t *client) {
    ...
    json_array_foreach(json_loaded_nodes, json_loaded_nodes_index, json_loaded_node) {
        ...
        res = send_http_request(&request, &response, node.ip_address, node.port, ENDPOINT_CONNECTION_CHECK_HTTP_METHOD, ENDPOINT_CONNECTION_CHECK_URL_PATH, NULL, client->request_timeout);
    ...
    res = send_http_request(&request, &response, CLIENT_DEFAULT_IP_ADDRESS, client->port, ENDPOINT_NODE_REMOVE_HTTP_METHOD, ENDPOINT_NODE_REMOVE_URL_PATH, json_offline_nodes, client->request_timeout);
...
int retrieve_known_nodes_connections(const client_settings_t *client) {
    ...
    json_array_foreach(json_loaded_nodes, json_loaded_nodes_index, json_loaded_node) {
        ...
        res = send_http_request(&request, &response, node.ip_address, node.port, ENDPOINT_NODE_RETRIEVE_HTTP_METHOD, ENDPOINT_NODE_RETRIEVE_URL_PATH, client->json_node, client->request_timeout);
        ...
        if (res == U_OK && response.status == 200) {
            ...
            res = send_http_request(&request, &response, CLIENT_DEFAULT_IP_ADDRESS, client->port, ENDPOINT_NODE_ADD_HTTP_METHOD, ENDPOINT_NODE_ADD_URL_PATH, response_nodes, client->request_timeout);
...
int contact_dns_server(const client_settings_t *client) {
    ...
    res = send_http_request(&request, &response, PRIMARY_DNS_SERVER_IP_ADDRESS, PRIMARY_DNS_SERVER_PORT, ENDPOINT_NODE_RETRIEVE_HTTP_METHOD, ENDPOINT_NODE_RETRIEVE_URL_PATH, client->json_node, client->request_timeout);
    ...
    if (res == U_OK && response.status == 200) {
        ...
        res = send_http_request(&request, &response, CLIENT_DEFAULT_IP_ADDRESS, client->port, ENDPOINT_NODE_ADD_HTTP_METHOD, ENDPOINT_NODE_ADD_URL_PATH, json_response_nodes, client->request_timeout);
